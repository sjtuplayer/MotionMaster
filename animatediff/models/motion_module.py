...
class TemporalTransformer3DModel(nn.Module):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None):
        ...
        hidden_states = rearrange(hidden_states, "b c f h w -> (b f) c h w")  # 2,320,16,64,64-> 32,320,64,64
        ...
        hidden_states = hidden_states.permute(0, 2, 3, 1).reshape(batch, height * weight,
                                                                  inner_dim)  # 32,320,64,64->32,4096,320
        hidden_states = self.proj_in(hidden_states)

        # Transformer Blocks
        for block in self.transformer_blocks:
            hidden_states = block(hidden_states, encoder_hidden_states=encoder_hidden_states,
                                  video_length=video_length)  # 调用时序模块
...
class VersatileAttention(CrossAttention):
    def __init__(
        ...
        self.last_attn_map2 = [0 for i in range(self.ddim_step)]
        self.last_value = [0 for i in range(self.ddim_step)]
        self.current_step = 0

    def extra_repr(self):
        return f"(Module Info) Attention_Mode: {self.attention_mode}, Is_Cross_Attention: {self.is_cross_attention}"

    def ht_attention(self, query, key, value, attention_mask=None):
        if self.upcast_attention:
            query = query.float()  # (b*h*w)*f*c
            key = key.float()
        ...
        if self.current_step == inversion_step:
            ...
            file_path = os.path.join(savedir, 'tmp-save', f'{self.current_step}-{self.id % 42}.pt')
        ...
        attention_probs = attention_probs.to(value.device)
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None,
        ...
        if self._use_memory_efficient_attention_xformers:
            ...
            if self._slice_size is None or query.shape[0] // self._slice_size == 1:
                hidden_states = self.ht_attention(query, key, value, attention_mask)
            else:
