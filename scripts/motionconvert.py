...
from animatediff.pipelines.pipeline_motionconvert import motionconvert_Pipeline
from animatediff.utils.util import save_videos_grid
from animatediff.utils.util import load_weights
from diffusers.utils.import_utils import is_xformers_available
from animatediff.utils.util import read_video, read_gif, read_file, resize_video
...
def main(args):
    *_, func_args = inspect.getargvalues(inspect.currentframe())
    ...
    for model_idx, model_config in enumerate(config):
        ...
        pipeline = motionconvert_Pipeline(
        ...
        prompts1 = model_config.prompt1
        prompts2 = model_config.prompt2
        n_prompts = list(model_config.n_prompt) * len(prompts1) if len(
            model_config.n_prompt) == 1 else model_config.n_prompt
        random_seeds = model_config.get("seed", [-1])
        random_seeds = [random_seeds] if isinstance(random_seeds, int) else list(random_seeds)
        random_seeds = random_seeds * len(prompts1) if len(random_seeds) == 1 else random_seeds
        ...
        for prompt_idx, (prompt1, n_prompt, random_seed) in enumerate(zip(prompts1, n_prompts, random_seeds)):
            for prompt2 in prompts2:
                ...
                print(f"sampling {prompt2} ...")

                file_name = model_config.video_name
                file_name = file_name[0]
                video_tensor = read_file(file_name)
                video_tensor = resize_video(video_tensor)
                ...
                prompt = "-".join((prompt2.replace("/", "").split(" ")[:10]))
